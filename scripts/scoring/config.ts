...
export const METRICS: MetricDefinition[] = [
  ...
  // Safety depth from inspection/penalties summary columns
  {
    key: "deficiency_count",
    column: "deficiency_count",
    providerTypes: ["nursing_home"],
    higherIsBetter: false,
    description: "Total health deficiencies in current survey cycle",
  },
  {
    key: "has_immediate_jeopardy",
    column: "has_immediate_jeopardy",
    providerTypes: ["nursing_home"],
    higherIsBetter: false,
    description: "Immediate jeopardy deficiency flag (boolean)",
  },
  {
    key: "total_penalties_amount",
    column: "total_penalties_amount",
    providerTypes: ["nursing_home"],
    higherIsBetter: false,
    description: "Total CMP fines in last 3 years (USD)",
  },
  {
...
export const DEFAULT_WEIGHTS: Record<ProviderType, Record<string, number>> = {
  nursing_home: {
    ...
    // Claims/MDS quality outcomes (long-stay)
    hosp_ls_per_1k: 0.06, // Hospitalizations per 1,000 resident-days (invert)
    ed_ls_per_1k: 0.04,   // ED visits per 1,000 resident-days (invert)
    // Outcome proxies (validation only)

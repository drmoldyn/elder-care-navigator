...
function toSlug(params: { cityState: string[] }): string {
  return (params.cityState || []).join("/") || "";
}
      ...
      <div className="mx-auto flex min-h-[70vh] w-full max-w-6xl flex-col gap-6 md:gap-8 px-4 md:px-6 py-10 md:py-14 relative z-10">
        <header className="space-y-3 md:space-y-4">
          <nav className="text-xs md:text-sm text-gray-600">
            ...
            <span className="text-gray-900 font-medium">{city}, {state}</span>
          </nav>
        ...
        {hasData && (
          <section className="grid gap-3 md:gap-4 grid-cols-2 md:grid-cols-4">
            ...
            {Object.entries(providerTypeBreakdownDisplay).slice(0, 2).map(([type, count]) => (
              <div key={type} className="rounded-xl border border-slate-200 bg-white/80 p-4 md:p-5 shadow-sm">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white/85 p-4 md:p-6 shadow-sm">
            ...
            <div className="space-y-3 md:space-y-4">
              {topFacilitiesDisplay.map((facility, index) => (
                <div key={facility.id} className="rounded-lg border border-slate-200 bg-white/90 p-4 md:p-5 hover:border-sunset-orange/40 hover:shadow-md transition-all">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 md:gap-4">
                    ...
                    <div className="flex-shrink-0 text-right">
                      <div className={`text-2xl md:text-3xl font-bold ${getScoreColor(facility.sunsetwell_score)}`}>{facility.sunsetwell_score.toFixed(0)}</div>
                      <div className="text-xs md:text-sm text-slate-500">{getScoreLabel(facility.sunsetwell_score)}</div>
                      <Link href={`/results/compare?facilities=${facility.id}`} className="mt-2 inline-block text-xs md:text-sm text-sunset-orange hover:underline">View details →</Link>
                    </div>
                  </div>
                </div>
              ))}
